...
<section class="frame-section">
    <section class="frame-left">
        <nav class="frame-nav">
            <ul>
                <%var menuGroup=_.groupBy(menus,function(menu){return
                menu.parent==null||menu.parent==-1?"root":menu.parent});
                var createMenuItem = function(m){_.each(_.sortBy(m,function(m){return m.position}), function(menu){%>
                <li navid="<%-menu.id%>"><a href="<%-menu.path?menu.path:'#'%>"<%if(menu.description){%>
                    title="<%-menu.description%>"<%}%>><b class="<%-menu.icon%> nav-mask"></b><%-menu.text%></a>
