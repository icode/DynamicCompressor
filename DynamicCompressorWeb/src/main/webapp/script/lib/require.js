...
(function (global) {
        ...
        return (context = {
            ...
            onScriptLoad:function (evt) {
                ...
                if (evt.type === 'load' ||
                    ...
                    //动态压缩支持
                    var data = getScriptData(evt), ids = data.id.split(',');
                    for (var i = 0; i < ids.length; i++)
                        context.completeLoad(ids[i]);
                }
            },
    ...
    req.load = function (context, moduleName, url) {
        var compressorCfg = context.config.compressor;
        if (compressorCfg && compressorCfg.enabled === true) {
            if (compressorCfg.merge === true) {
                if (timer) {
                    clearTimeout(timer);
                }
                url = encodeUrl(url, compressorCfg.compress, compressorCfg.browserCondition);
